const { insertPostImage } = require("../../repositories/posts");
...
const createPost = async (req, res, next) => {
  try {
    ...
    const insertedPostId = await insertPostImage({
      image,
      description,
      userId,
    });
    ...
    for (const imageName of images) {
      ...
      const image = await processAndSaveImage(imageName.data);

      // Insertamos la información de la imagen en la DB. El repositorio nos retorna el id de la imagen insertada
      const insertedImageId = await insertPostImage(image, description, userId);
